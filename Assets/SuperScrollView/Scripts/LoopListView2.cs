...
namespace SuperScrollView
{
    ...
    public class LoopListView2 : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
    {
        ...
        bool mIsNeedSnapMove = false;
        float mTargetSnapVal = 0;
        float mCurSnapVal = 0;
        ...
        public System.Action<LoopListView2, LoopListViewItem2> mOnSnapTargetChanged = null;
        int mCurSnapTargetItemIndex = -1;
        Vector2 mAdjustedVec;
        bool mNeedAdjustVec = false;
        int mLeftSnapUpdateExtraCount = 0;
        [SerializeField]
        ...
        */
        ...
        void AdjustPanelPos()
        {
            ...
            if (mArrangeType == ListItemArrangeType.TopToBottom)
            {
                if (contentSize <= viewPortSize)
                {
                    ...
                    mItemList[0].CachedRectTransform.localPosition = Vector3.zero;
                    UpdateAllShownItemsPos();
                    return;
                }
        ...
        void UpdateSnapVertical(bool immediate = false)
        {
            ...
            Vector3 pos = mContainerTrans.localPosition;
            float curY = pos.y;
            bool needCheck = (pos.y != mLastFrameContainerPos.y);
            if (!needCheck)
            {
            ...
            if (needCheck)
            {
                ...
                if (curIndex >= 0)
                {
                    if (mItemList[curIndex].ItemIndex != mCurSnapTargetItemIndex)
                    {
                        if (mOnSnapTargetChanged != null)
                        {
                            mOnSnapTargetChanged(this,mItemList[curIndex]);
                        }
                    }
                    mCurSnapTargetItemIndex = mItemList[curIndex].ItemIndex;
                    mTargetSnapVal = targetSnapVal;
                    mCurSnapVal = 0;
                    mIsNeedSnapMove = true;
                }
                else
                {
                    mCurSnapTargetItemIndex = -1;
                    mIsNeedSnapMove = false;
                }
            }
            bool canSnap = true;
            ...
            if (canSnap && mIsNeedSnapMove && !mIsDraging && v < mSnapVecThreshold)
            {
                if(v > 0)
                {
                    mScrollRect.StopMovement();
                }
                float old = mCurSnapVal;
                mCurSnapVal = Mathf.SmoothDamp(mCurSnapVal, mTargetSnapVal, ref mSmoothDumpVel, mSmoothDumpRate);
                float dt = mCurSnapVal - old;
                
                if (immediate || Mathf.Abs(mTargetSnapVal - mCurSnapVal) < mSnapFinishThreshold)
                {
                    mIsNeedSnapMove = false;
                    pos.y = pos.y + mTargetSnapVal - mCurSnapVal;
                    if(mOnSnapItemFinished != null)
                    {
                        LoopListViewItem2 targetItem = GetShownItemByItemIndex(mCurSnapTargetItemIndex);
                        if(targetItem != null)
                        {
        ...
        void UpdateSnapHorizontal(bool immediate = false)
        {
            ...
            Vector3 pos = mContainerTrans.localPosition;
            float curX = pos.x;
            bool needCheck = (pos.x != mLastFrameContainerPos.x);
            if(!needCheck)
            {
                if(mLeftSnapUpdateExtraCount > 0)
                {
            ...
            if (canSnap && mIsNeedSnapMove && !mIsDraging && v < mSnapVecThreshold)
            {
                ...
                {
                    mIsNeedSnapMove = false;
                    pos.x = pos.x + mTargetSnapVal - mCurSnapVal;
                    if (mOnSnapItemFinished != null)
                    {
                        LoopListViewItem2 targetItem = GetShownItemByItemIndex(mCurSnapTargetItemIndex);
                        if (targetItem != null)
        ...
        bool UpdateForVertList(float distanceForRecycle0,float distanceForRecycle1,float distanceForNew0, float distanceForNew1)
        {
            ...
            if (mArrangeType == ListItemArrangeType.TopToBottom)
            {
                ...
                if (mItemList.Count == 0)
                {
                    ...
                    mItemList.Add(newItem);
                    newItem.CachedRectTransform.localPosition = new Vector3(0, pos, 0);
                    UpdateContentSize();
                    return true;
                }
                LoopListViewItem2 tViewItem0 = mItemList[0];
                ...
                if (!mIsDraging && downPos0.y - mViewPortRectLocalCorners[1].y > distanceForRecycle0)
                {
                ...
                Vector3 topPos1 = mViewPortRectTransform.InverseTransformPoint(mItemWorldCorners[1]);
                Vector3 downPos1 = mViewPortRectTransform.InverseTransformPoint(mItemWorldCorners[0]);
                if (!mIsDraging && mViewPortRectLocalCorners[0].y - topPos1.y > distanceForRecycle1)
                {
                ...
                if (mViewPortRectLocalCorners[0].y - downPos1.y < distanceForNew1)
                {
                    ...
                    if (nIndex <= mCurReadyMaxItemIndex || mNeedCheckNextMaxItem)
                    {
                        ...
                        else
                        {
                            ...
                            newItem.CachedRectTransform.localPosition = new Vector3(0, y, 0);
                            UpdateContentSize();
                            CheckIfNeedUpdataItemPos();
            ...
            else
            {
                ...
                if (!mIsDraging && mViewPortRectLocalCorners[0].y - topPos0.y > distanceForRecycle0)
                {
                    mItemList.RemoveAt(0);
                    RecycleItemTmp(tViewItem0);
                    if (!mSupportScrollBar)
                ...
                Vector3 topPos1 = mViewPortRectTransform.InverseTransformPoint(mItemWorldCorners[1]);
                Vector3 downPos1 = mViewPortRectTransform.InverseTransformPoint(mItemWorldCorners[0]);
                if (!mIsDraging && downPos1.y - mViewPortRectLocalCorners[1].y > distanceForRecycle1)
                {
                    mItemList.RemoveAt(mItemList.Count - 1);
                    RecycleItemTmp(tViewItem1);
                    if (!mSupportScrollBar)
        ...
        bool UpdateForHorizontalList(float distanceForRecycle0, float distanceForRecycle1, float distanceForNew0, float distanceForNew1)
        {
            ...
            if (mArrangeType == ListItemArrangeType.LeftToRight)
            {
                ...
                if (mItemList.Count == 0)
                {
                    ...
                    mItemList.Add(newItem);
                    newItem.CachedRectTransform.localPosition = new Vector3(pos, 0, 0);
                    UpdateContentSize();
                    return true;
                }
                LoopListViewItem2 tViewItem0 = mItemList[0];
                ...
                if (!mIsDraging && mViewPortRectLocalCorners[1].x - rightPos0.x > distanceForRecycle0)
                {
                    mItemList.RemoveAt(0);
                    RecycleItemTmp(tViewItem0);
                    if (!mSupportScrollBar)
                ...
                Vector3 leftPos1 = mViewPortRectTransform.InverseTransformPoint(mItemWorldCorners[1]);
                Vector3 rightPos1 = mViewPortRectTransform.InverseTransformPoint(mItemWorldCorners[2]);
                if (!mIsDraging && leftPos1.x - mViewPortRectLocalCorners[2].x> distanceForRecycle1)
                {
                    mItemList.RemoveAt(mItemList.Count - 1);
                    RecycleItemTmp(tViewItem1);
                    if (!mSupportScrollBar)
                ...
                if (rightPos1.x - mViewPortRectLocalCorners[2].x < distanceForNew1)
                {
                    ...
                    if (nIndex <= mCurReadyMaxItemIndex || mNeedCheckNextMaxItem)
                    {
                        ...
                        else
                        {
                            ...
                            newItem.CachedRectTransform.localPosition = new Vector3(x, 0, 0);
                            UpdateContentSize();
                            CheckIfNeedUpdataItemPos();
            ...
            else
            {
                ...
                if (!mIsDraging && leftPos0.x - mViewPortRectLocalCorners[2].x > distanceForRecycle0)
                {
                    mItemList.RemoveAt(0);
                    RecycleItemTmp(tViewItem0);
                    if (!mSupportScrollBar)
                ...
                Vector3 leftPos1 = mViewPortRectTransform.InverseTransformPoint(mItemWorldCorners[1]);
                Vector3 rightPos1 = mViewPortRectTransform.InverseTransformPoint(mItemWorldCorners[2]);
                if (!mIsDraging && mViewPortRectLocalCorners[1].x - rightPos1.x > distanceForRecycle1)
                {
                    mItemList.RemoveAt(mItemList.Count - 1);
                    RecycleItemTmp(tViewItem1);
                    if (!mSupportScrollBar)
        ...
        void UpdateAllShownItemsPos()
        {
            ...
            if (mArrangeType == ListItemArrangeType.TopToBottom)
            {
                ...
                for (int i = 0; i < count; ++i)
                {
                    ...
                    item.CachedRectTransform.localPosition = new Vector3(0, curY, 0);
            ...
            else if (mArrangeType == ListItemArrangeType.LeftToRight)
            {
                ...
                {
                    LoopListViewItem2 item = mItemList[i];
                    item.CachedRectTransform.localPosition = new Vector3(curX, 0, 0);
