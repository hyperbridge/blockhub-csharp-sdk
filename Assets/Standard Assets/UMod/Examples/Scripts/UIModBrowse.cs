...
namespace UMod.Example
{
    public class UIModBrowse : MonoBehaviour
    {
        ...
        public void OnLoadClicked()
        {
            if (active != null)
            {
                Mod.LoadMod(ModDirectory.GetModPath(active.ModName));
            }
        }
        ...
        private void CreateUICell(GameObject owner, IModInfo mod)
        {
            ...
            // Fill out the fields
            element.Name = mod.ModName;
            element.Version = mod.ModVersion;

            // Use a relative path
            string relative = ModDirectory.GetModPath(mod.ModName).ToString();
