...
#region Color Properties
	...
	private Color lightPrimaryColor {
		get { return new Color(207 / 255f, 216 / 255f, 220 / 255f); }
	}

	private Color accentColor {
		get { return new Color(255 / 255f, 193 / 255f, 7 / 255f); }
	}

	private Color primaryTextColor {
		get { return new Color(33 / 255f, 33 / 255f, 33 / 255f); }
	}

	private Color secondaryTextColor {
		get { return new Color(117 / 255f, 117 / 255f, 117 / 255f); }
	}

	private Color dividerColor {
		get { return new Color(189 / 255f, 189 / 255f, 189 / 255f); }
	}
	...
	private GUIStyle fieldStyle {
		get {
			if (_fieldStyle == null) {
				...
				_fieldStyle.font = Resources.Load<Font>("Fonts/Fira Mono/FiraMono-Regular");
...
#region Save Methods
	new public JsonFile Save(FileInfo info = null) {
		if (info != null)	// Save as
			this.info = info;
		if (this.info != null) {	// Save
			using (FileStream stream = File.Create(this.info.FullName)) {}
			File.WriteAllText(this.info.FullName, JsonConvert.SerializeObject(data));
		}
		else
...
#region Render Methods
	...
	private Rect RenderIndex(Rect pos, int index) {
		Rect iconPos = RenderFieldIcon(pos, keyIcon, true);
		GUI.backgroundColor = lightPrimaryColor;
		Rect namePos = iconPos;
		namePos.x = iconPos.xMax + bufferSize;
		namePos.width = fieldWidth;
		EditorGUI.LabelField(namePos, index.ToString(), fieldStyle);
		GUI.backgroundColor = Color.white;
		namePos.xMax += bufferSize;
		SetWorkspace(namePos);
		return namePos;
