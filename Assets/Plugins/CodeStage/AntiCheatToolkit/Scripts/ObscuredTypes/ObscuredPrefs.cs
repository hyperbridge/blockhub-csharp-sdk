...
namespace CodeStage.AntiCheat.ObscuredTypes
{
	...
	public static class ObscuredPrefs
	{
		...
		public static void ForceLockToDeviceInit()
		{
			...
			{
				Debug.LogWarning(ACTkConstants.LOG_PREFIX + "ObscuredPrefs.ForceLockToDeviceInit() is called, but device ID is already obtained!");
			}
		}
		...
		public static Vector2 GetVector2(string key, Vector2 defaultValue)
		{
			...
			if (unobscuredMode)
			{
				var values = ReadUnobscured(key, defaultValue).Split(DATA_SEPARATOR[0]);
				float x;
				float y;
		...
		private static Vector2 DecryptVector2Value(string key, string encryptedInput, Vector2 defaultValue)
		{
			if (encryptedInput.IndexOf(DEPRECATED_RAW_SEPARATOR) > -1)
			{
				var deprecatedValue = DeprecatedDecryptValue(encryptedInput);
				if (deprecatedValue == "") return defaultValue;
				var values = deprecatedValue.Split(DATA_SEPARATOR[0]);
				float x;
				float y;
				float.TryParse(values[0], out x);
				float.TryParse(values[1], out y);
		...
		{
			var result = PlayerPrefs.GetString(encryptedKey, RAW_NOT_FOUND);

			if (result == RAW_NOT_FOUND)
			{
				if (PlayerPrefs.HasKey(key))
				{
					Debug.LogWarning(ACTkConstants.LOG_PREFIX + "Are you trying to read regular PlayerPrefs data using ObscuredPrefs (key = " + key + ")?");
				}
			}
			return result;
		}
...
#else
			Debug.LogError(ACTkConstants.LOG_PREFIX + "Looks like you forced ACTK_PREVENT_READ_PHONE_STATE flag, but still use LockToDevice feature. It will work incorrect!");
#endif
			return id;
		}
	...
	internal class BitconverterExt
	{
		...
		{
			if (bytes.Length != 16)
				throw new Exception(ACTkConstants.LOG_PREFIX + "A decimal must be created from exactly 16 bytes");
			var bits = new int[4];
